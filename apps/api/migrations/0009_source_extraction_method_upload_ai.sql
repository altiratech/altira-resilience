CREATE TABLE source_document_files_next_upload_ai (
  ...
  extraction_method TEXT CHECK (
    extraction_method IN ('upload_native', 'upload_ai', 'manual_native', 'queued_native', 'queued_ai')
  ),
...
INSERT INTO source_document_files_next_upload_ai (
  document_id,
  uploaded_file_name,
  mime_type,
  byte_size,
...
ALTER TABLE source_document_files_next_upload_ai RENAME TO source_document_files;

CREATE INDEX IF NOT EXISTS idx_source_document_files_updated_at ON source_document_files(updated_at DESC);

CREATE TABLE source_document_extraction_jobs_next_upload_ai (
  id TEXT PRIMARY KEY,
  ...
  attempted_method TEXT CHECK (
    attempted_method IN ('upload_native', 'upload_ai', 'manual_native', 'queued_native', 'queued_ai')
  ),
...
INSERT INTO source_document_extraction_jobs_next_upload_ai (
  id,
  document_id,
  status,
  attempt_count,
...
ALTER TABLE source_document_extraction_jobs_next_upload_ai RENAME TO source_document_extraction_jobs;
